...
public class BirthdayProcessor implements Runnable {
	...
	public void run() {
		...
		User me = facebookHandler
				.getObject(facebookHandler.getUserId(), User.class);
		...
		for (User friend : friends) {
			...
			if (user.getBirthday() != null
				...
				try {
					logger
					.info(String.format("Wishing %s happy birthday", user.getName()));
					Post post = new Post();
					post.setFrom(me);
					post.setMessage(String.format("Happy Birthday %s !",
							user.getFirstName()));
